...
def collect_data(load_percent):
    ...
    try:
        ...
        time.sleep(2)
        socketio.emit('status', {'message': 'Connected to Arduino. Recording...'})
        ...
        while time.time() - t_start < RECORD_SECONDS and recording:
            try:
                ...
                if is_conc:
                    ...
                    if prev_conc:
                        ...
                        if duration > REP_THRESH:
                            ...
                            rep_count += 1
                            socketio.emit('rep_detected', {'rep_count': rep_count - 1})
                ...
                if len(vt) > window_size:
                    ...
                    if np.max(recent_vt) - np.min(recent_vt) < VELOCITY_CHANGE_THRESH:
                        socketio.emit('status', {'message': 'No significant motion for 4 sec. Stopping early.'})
                        break
                
                idx += 1
                
                # Emit live data every 5 samples
                if idx % 5 == 0:
        ...
        trend_line = []
        if len(peakVelocity) > 1:
            x = np.arange(1, rep_count + 1)
            p = np.polyfit(x, peakVelocity, 1)
            trend_line = np.polyval(p, x).tolist()
        
        # ================== EMIT RESULTS ==================
        socketio.emit('results', {
            'reps_completed': reps_completed,
            'mean_velocity_last_rep': round(mean_vel_last_rep, 3),
            'velocity_loss': round(velocityLoss, 1),
...
def start_recording():
    data = request.json
    load_percent = data.get('load_percent', 80)
    
    if not recording:
        thread = threading.Thread(target=collect_data, args=(load_percent,))
        thread.start()
