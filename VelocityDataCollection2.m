...
% Set Features at each rep
velocityRepLoss = zeros(rep_count, 1);
peakvel_slope = zeros(rep_count, 1);
actual_rir = zeros(rep_count, 1);
...
for i = 1:rep_count
    ...
    if i > 1 
        velocityRepLoss(i) = avgVelocity(1) - avgVelocity(i);
        velocityRepLoss(i) = velocityRepLoss(i) * 100 / avgVelocity(1);

        peakvel_slope_poly = polyfit((1:i), peakVelocity(1:i), 1);
        peakvel_slope(i) = peakvel_slope_poly(1);

        actual_rir(i) = rep_count - i;
    end 
end 
...
xlabel("Rep #");
ylabel("Velocity (m/s)");
title("Mean + Peak Velocity Change");
legend('Mean Vel', "Peak Vel", "Peak Best Fit");
grid;
hold on;

%create a display a table of each rep window
repSummary = table( (1:rep_count)', avgVelocity, velocityRepLoss, actual_rir, peakvel_slope,...
    'VariableNames',{'Rep#', 'Rep Velocity', 'Mean Vel Loss', 'RIR', 'Peak Vel Slope'});
disp(repSummary);
